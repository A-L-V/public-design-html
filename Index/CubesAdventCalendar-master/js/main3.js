...
import desyng from '../css/ramdomstyle.json' with {type: "json"};
...
; (function (window) {
	...
	function createCubes() {
		const calendarGallery = document.getElementsByClassName('calendar')[0];
		

		data.forEach(i => {
			let newCube = document.createElement('div')
			newCube.setAttribute("class", "cube")
			const diseñoAleatorio = desyng.cube_mouse_hover_color_background[Math.floor(Math.random() * desyng.cube_mouse_hover_color_background.length)];
			if (currentDay >= i.day && currentMonth == 12) {
				
				newCube.setAttribute("data-title", i.content__title)
				newCube.setAttribute("data-bg-color", diseñoAleatorio)

			} else {
				newCube.setAttribute("data-title", "Coming soon")
				newCube.setAttribute("data-bg-color", "#000000")
				newCube.setAttribute("data-inactive", true)
			}

			calendarGallery.appendChild(newCube);
		})
	}
	function createContentBlocks() {

		const calendarGallery = document.querySelector('.content');

		data.sort((a, b) => b.day - a.day);

		data.forEach(i => {
			// Usamos ` ` (backticks) para escribir HTML dentro de JS e inyectar variables con ${}
			...
			`;
				...
				calendarGallery.insertAdjacentHTML('afterbegin', html);
